...
function TamilNadu() {
  ...
  useEffect(() => {
    axios.get("http://localhost:3001/tamilnadu")
      .then(response => {
...
const Moving=()=>{
  window.location.href='/addtamilnadu';
}
    ...
    <div>
      {
        name == 'Yuvasakthi' && password == 'Yuvasakthi@2024' && n2 == 'true'? (  <div>
          <h2 className="ps-5 pt-3">TamilNadu Packages</h2>
          <div className="d-flex flex-wrap justify-content-center align-items-center">
            {array1.map((k, index) => (
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <Link to={`/updatetamilnadu/${k._id}`} className="btn btn-warning ms-3 mb-2"><FaPenSquare/></Link>
